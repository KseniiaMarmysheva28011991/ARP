...
sc_df = pd.read_csv('SC_raw_results.csv')
equivalence_df = pd.read_csv('SC_vs_Initial_queries_row_comparison.csv')
...
merged_df = pd.merge(sc_df, initial_df, on='join_key', suffixes=('_sc', '_initial'))
...
def perform_tests(row):
    ...
    optimized_query = [row[f'time_pg_{i}_sc'] for i in range(1, 6)]
    ...
    variance_sc = np.var(optimized_query, ddof=1)
    ...
    # Metric differences
    avg_pg_time_diff = row['avg_pg_time_sc'] - row['avg_pg_time_initial']
    median_pg_time_diff = row['median_pg_time_sc'] - row['median_pg_time_initial']
    avg_cost_diff = row['avg_cost_sc'] - row['avg_cost_initial']
    avg_rows_diff = row['avg_rows_sc'] - row['avg_rows_initial']

    return pd.Series({
        'normality_initial': normality_orig,
        'normality_sc': normality_opt,
        ...
        'avg_pg_time_diff_sc_initial': avg_pg_time_diff,
        'median_pg_time_diff_sc_initial': median_pg_time_diff,
        'avg_cost_diff_sc_initial': avg_cost_diff,
        'avg_rows_diff_sc_initial': avg_rows_diff
    })
...
# === Save result ===
final_df.to_csv('SC_vs_Initial_queries_experiment_data.csv', index=False)
print("File saved: SC_vs_Initial_queries_experiment_data.csv")
